...
export const DrugTypeModel = types
  ...
  .views(self => ({
    ...
    getEntriesForDate(date) {
      ...
      return _.filter(self.entries, t =>
        moment(t.timestamp).isSame(dateMoment, "day")
      );
    },
    getAverageTimeBetweenEntries(count) {
      ...
      let timeBetween =
        (_.last(sortedEntries).timestamp -
          sortedEntries[count - self.entries.length].timestamp) /
...
const DomainStore = types
  ...
  .views(self => ({
    ...
    searchDrugsByName(name) {
      ...
      return _.filter(self.drugs, t =>
        t.name.toUpperCase().startsWith(name.toUpperCase())
      );
    },
    getEntryById(id) {
